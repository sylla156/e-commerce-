...
class insere extends connexion
{
    ...
    public function insereUser($reference,string $nom, string $prenom, int $tel, string $email, string $password)
    {

        $this->connecter();
        $go = $this->db->prepare("INSERT INTO `personne` (`id`, `nom`, `prenom`,`tel`, `email`, `pass`) VALUES (?,?,?,?,?,?)");
